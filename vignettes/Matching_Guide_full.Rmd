...
author: \href{mailto:thomas.leavitt@baruch.cuny.edu}{Thomas Leavitt} and \href{mailto:luke_miratrix@gse.harvard.edu }{Luke Miratrix}
date: ""
output: 
  pdf_document:
...
---
```{r setup-matchingguide, include=TRUE, message=FALSE}
# Prefer the installed package; otherwise source helpers from the local R/ folder when knitting the vignette standalone.
if (requireNamespace("matchingguide", quietly = TRUE)) {
  library(matchingguide)
} else {
  message("matchingguide not installed; sourcing local helper functions.")
  source("../R/utils.R")
  source("../R/data.R")
}
```
...
# Insert matching pipeline flowchart (PDF) into document
knitr::include_graphics("flowchart/matching_pipeline_flowchart.pdf")
...
# Install packages (only run these lines if you don't already have them installed)
...
library(haven)      # For reading and writing Stata (.dta) files
library(dplyr)      # For data manipulation functions like mutate(), if_else(), etc.
library(lubridate)  # For handling and converting date-time objects (ym(), months())
library(readr)      # For reading and writing .rds files (read_rds(), write_rds())

# Load the Stata dataset into \texttt{R} as a dataframe called 'data'
data <- read_dta(file = "qjps_8051_supp/peace_pre_match.dta")

# Convert Stata monthly numeric dates to actual year-month dates in R
data <- data |>
  mutate(
    # date0 in Stata's %tm format (monthly), starting from January 1960
    date0 = ym("1960-01") + months(date0),
    
    # date1 in Stata's %tm format (monthly), starting from January 1960
    date1 = ym("1960-01") + months(date1)
  )

# Recode binary geographic region variables into one factor variable
data <- data |>
  mutate(
    region = NA_character_,
    region = if_else(eeurop == 1, "eeurop", region),
    region = if_else(lamerica == 1, "lamerica", region),
    region = if_else(asia == 1, "asia", region),
    region = if_else(ssafrica == 1, "ssafrica", region),
    region = if_else(nafrme == 1, "nafrme", region),
    region = factor(region)
  )

# Recode ethnic fractionalization index to 0-1 range
data <- data |>
  mutate(
    ethfrac = ethfrac / 100
  )
...
write_rds(x = data, file = "peace_pre_match.rds")

```

```{r}

# Load the pre-created dataset with converted dates and region factor
data <- read_rds(file = "peace_pre_match.rds")
