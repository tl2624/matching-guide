...
# Load the here package for consistent file paths
library(here)

# Insert the matching pipeline flowchart (PDF) into the document
# Use the full path from the project root (the file is inside vignettes/fig/)
include_graphics(here("vignettes", "fig", "matching_pipeline_flowchart.pdf"))
...
# Packages (install first if needed: install.packages("<name>"))
library(readr)
library(here)

# Paths (anchored at project root via here::i_am() in setup)
rds_path   <- here("data", "peace_pre_match.rds")            # the RDS you want to read
build_path <- here("data-raw", "build_peace_pre_match.R")    # the script that creates it

# Build the RDS if it's missing
