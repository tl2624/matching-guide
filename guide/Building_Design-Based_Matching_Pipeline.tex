...
dates have been converted to R's year-month format, the geographic
region indicators recoded into a single factor variable
(\texttt{region}), and the ethnic fractionalization index
(\texttt{ethfrac}) rescaled from 0-100 to 0-1. The command below loads
this pre-created dataset (\texttt{peace\_pre\_match.RData}) into the
\texttt{R} environment as an object named \texttt{data}.
...
\NormalTok{data }\OtherTok{\textless{}{-}} \FunctionTok{read\_rds}\NormalTok{(}\AttributeTok{file =} \StringTok{"../data/peace\_pre\_match.rds"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{expanded\_covs }\OtherTok{\textless{}{-}} \FunctionTok{c}\NormalTok{(}
  ...
  \FunctionTok{setdiff}\NormalTok{(}\AttributeTok{x =}\NormalTok{ covs, }\AttributeTok{y =} \StringTok{"region"}\NormalTok{), }\CommentTok{\# drops "region" from the covariate list}
...
\CommentTok{\# Check that p\_scores equals logistic(lin\_cov\_inds)}
...
\NormalTok{euc\_dist\_polity\_cal\_2 }\OtherTok{\textless{}{-}} \FunctionTok{match\_on}\NormalTok{(}\AttributeTok{x =}\NormalTok{ UN }\SpecialCharTok{\textasciitilde{}}\NormalTok{ bwplty2,}
...
\NormalTok{euc\_dist\_polity\_cal\_3 }\OtherTok{\textless{}{-}} \FunctionTok{match\_on}\NormalTok{(}\AttributeTok{x =}\NormalTok{ UN }\SpecialCharTok{\textasciitilde{}}\NormalTok{ bwplty2,}
...
\NormalTok{eth\_mat }\OtherTok{\textless{}{-}} \FunctionTok{match\_on}\NormalTok{(}
  ...
  \AttributeTok{caliper =} \FloatTok{0.35}\NormalTok{, }\CommentTok{\# caliper of 0.35}
  \AttributeTok{data =}\NormalTok{ data,}
  \AttributeTok{standardization.scale =} \ConstantTok{NULL}\NormalTok{,}
...
\NormalTok{bwgdp\_mat }\OtherTok{\textless{}{-}} \FunctionTok{match\_on}\NormalTok{(}
  ...
  \AttributeTok{caliper =} \DecValTok{2}\NormalTok{, }\CommentTok{\# caliper of 2}
  \AttributeTok{data =}\NormalTok{ data,}
  \AttributeTok{standardization.scale =} \ConstantTok{NULL}\NormalTok{,}
  \AttributeTok{method =} \StringTok{"euclidean"}
\NormalTok{)}
...
weighted by its effective sample size. We first reconstruct what the
treated outcomes would have been without treatment, as implied by the
null hypothesis \(\tau_h = 0\).
...
We next source a helper function that performs the rescaling described
above, ensuring that the sum of the rescaled outcomes across treated
units equals the harmonic-mean--weighted Difference-in-Means statistic.

\singlespacing

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Load the hm\_stat\_rescale() function}
\CommentTok{\# Check if the function is already loaded; if not, load it below}
\ControlFlowTok{if}\NormalTok{ (}\SpecialCharTok{!}\FunctionTok{exists}\NormalTok{(}\StringTok{"hm\_stat\_rescale"}\NormalTok{, }\AttributeTok{mode =} \StringTok{"function"}\NormalTok{)) \{}
  
  \CommentTok{\# If the function file exists locally, load it from the R/ folder}
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{file.exists}\NormalTok{(}\StringTok{"../R/hm\_stat\_rescale.R"}\NormalTok{)) \{}
    \FunctionTok{sys.source}\NormalTok{(}\AttributeTok{file =} \StringTok{"../R/hm\_stat\_rescale.R"}\NormalTok{,}
               \AttributeTok{envir =} \FunctionTok{environment}\NormalTok{())  }\CommentTok{\# make it available in the current session}
  
  \CommentTok{\# Otherwise, download and load it directly from the GitHub repository}
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
    \FunctionTok{sys.source}\NormalTok{(}\AttributeTok{file =} \FunctionTok{paste0}\NormalTok{(}
      \StringTok{"https://raw.githubusercontent.com/tl2624/"}\NormalTok{,}
      \StringTok{"matching{-}guide/main/R/hm\_stat\_rescale.R"}
\NormalTok{    ),}
...
We next apply the rescaling procedure and calculate the observed test
statistic.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Apply the rescaling function: adds a new column (.hm\_scaled)}
\CommentTok{\# and returns the full matched dataset with this rescaled outcome}
\NormalTok{data\_matched }\OtherTok{\textless{}{-}} \FunctionTok{hm\_stat\_rescale}\NormalTok{(}\AttributeTok{data =}\NormalTok{ data\_matched,}
                                \AttributeTok{outcome =}\NormalTok{ ldur\_tilde,}
                                \AttributeTok{treat =}\NormalTok{ UN,}
                                \AttributeTok{strata =}\NormalTok{ fm)}

\CommentTok{\# Observed HM{-}weighted diff{-}in{-}means statistic}
\NormalTok{obs\_stat }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(data\_matched}\SpecialCharTok{$}\NormalTok{ldur\_tilde\_hm\_scaled[data\_matched}\SpecialCharTok{$}\NormalTok{UN }\SpecialCharTok{==} \DecValTok{1}\NormalTok{])}
...
\NormalTok{sim\_sharp\_null\_dist }\OtherTok{\textless{}{-}} \FunctionTok{apply}\NormalTok{(}
  ...
  \AttributeTok{FUN =} \ControlFlowTok{function}\NormalTok{(x) \{}
    ...
    \FunctionTok{sum}\NormalTok{(data\_matched}\SpecialCharTok{$}\NormalTok{ldur\_tilde\_hm\_scaled[x }\SpecialCharTok{==} \DecValTok{1}\NormalTok{])}
\NormalTok{  \}}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# One{-}sided, upper p{-}value: proportion of simulated randomization stats \textgreater{}= observed}
...
\CommentTok{\# Exact one{-}sided, upper p{-}value: proportion of randomization stats \textgreater{}= observed}
...
stratified design using the approach of \citet{fogarty2018}. Rather than
defining the function within the script, we load a custom \texttt{R}
function (\texttt{fine\_strat\_var\_est()}) from the accompanying
replication files. If the local file is available, the function is
sourced directly from the \texttt{R/} folder; otherwise, it is
automatically retrieved from the online repository.

\singlespacing

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Load the fine\_strat\_var\_est() function}
\CommentTok{\# Check if the function is already loaded; if not, load it below}
\ControlFlowTok{if}\NormalTok{ (}\SpecialCharTok{!}\FunctionTok{exists}\NormalTok{(}\StringTok{"fine\_strat\_var\_est"}\NormalTok{, }\AttributeTok{mode =} \StringTok{"function"}\NormalTok{)) \{}
  
  \CommentTok{\# If the function file exists locally, load it from the R/ folder}
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{file.exists}\NormalTok{(}\StringTok{"../R/fine\_strat\_var\_est.R"}\NormalTok{)) \{}
    \FunctionTok{sys.source}\NormalTok{(}\AttributeTok{file =} \StringTok{"../R/fine\_strat\_var\_est.R"}\NormalTok{,}
               \AttributeTok{envir =} \FunctionTok{environment}\NormalTok{())  }\CommentTok{\# make it available in the current session}
...
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
    \FunctionTok{sys.source}\NormalTok{(}\AttributeTok{file =} \FunctionTok{paste0}\NormalTok{(}
      \StringTok{"https://raw.githubusercontent.com/tl2624/"}\NormalTok{,}
      \StringTok{"matching{-}guide/main/R/fine\_strat\_var\_est.R"}
\NormalTok{    ),}
    \AttributeTok{envir =} \FunctionTok{environment}\NormalTok{()}
\NormalTok{    )}
\NormalTok{  \}}
...
representing the estimated variance. After loading the function, we
compute the set-specific estimates and corresponding set sizes, and then
pass these two vectors as inputs to \texttt{fine\_strat\_var\_est()}.
...
tests. In this approach, we retain all null values that are not rejected
at the \(\alpha = 0.05\) level. However, for a two-sided test, we
instead use \(\alpha/2 = 0.025\) for each tail to account for testing
every null value twice (once against each alternative).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Set significance (alpha) level}
...
\NormalTok{    out }\OtherTok{\textless{}{-}} \FunctionTok{senstrat}\NormalTok{(}\AttributeTok{sc =}\NormalTok{ data\_matched}\SpecialCharTok{$}\NormalTok{ldur\_tilde\_hm\_scaled, }\CommentTok{\# outcome}
...
\begin{center}\includegraphics{Building_Design-Based_Matching_Pipeline_files/figure-latex/unnamed-chunk-49-1} \end{center}
\doublespacing
...
intuition for this approach, note that the following inverse probability
weighted (IPW) version of the Difference-in-Means is unbiased for the
ATE, \(\tau\): \begin{align} \label{eq: weighted diff-in-means}
...
hypothesis about the overall ATE across all matched sets, the procedure
replaces \(p(\bm{z}_s)\) with the upper bound from
\eqref{eq: prob bounds} whenever \(\hat{\tau}_s\) is greater than or
equal to the null value of the overall ATE, and with the lower bound
whenever \(\hat{\tau}_s\) is less than that null value. When testing the
...
value of the worst-case IPW Difference-in-Means --- defined under
...
IPW Difference-in-Means is less than or equal to the null when it is
true, then the worst-case IPW Difference-in-Means tends to fall at or
below the null rather than above it. In other words, the procedure
intentionally ``tilts'' the worst-case IPW Difference-in-Means in the
direction opposite the alternative, making the procedure conservative.
...
Difference-in-Means. When we standardize the worst-case IPW
...
worst-case IPW Difference-in-Means for a single matched set.

\singlespacing

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Load the worst\_case\_IPW() function}
\CommentTok{\# Check if the function is already loaded; if not, load it below}
\ControlFlowTok{if}\NormalTok{ (}\SpecialCharTok{!}\FunctionTok{exists}\NormalTok{(}\StringTok{"worst\_case\_IPW"}\NormalTok{, }\AttributeTok{mode =} \StringTok{"function"}\NormalTok{)) \{}
  
  \CommentTok{\# If the function file exists locally, load it from the R/ folder}
  \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{file.exists}\NormalTok{(}\StringTok{"../R/worst\_case\_IPW.R"}\NormalTok{)) \{}
    \FunctionTok{sys.source}\NormalTok{(}\AttributeTok{file =} \StringTok{"../R/worst\_case\_IPW.R"}\NormalTok{,}
               \AttributeTok{envir =} \FunctionTok{environment}\NormalTok{())  }\CommentTok{\# make it available in the current session}
...
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
    \FunctionTok{sys.source}\NormalTok{(}\AttributeTok{file =} \FunctionTok{paste0}\NormalTok{(}
      \StringTok{"https://raw.githubusercontent.com/tl2624/"}\NormalTok{,}
      \StringTok{"matching{-}guide/main/R/worst\_case\_IPW.R"}
\NormalTok{    ),}
    \AttributeTok{envir =} \FunctionTok{environment}\NormalTok{()}
\NormalTok{    )}
\NormalTok{  \}}
...
statistic, we take a weighted average of the set-specific values, using
weights proportional to the number of units in each set. Finally, we
standardize this overall statistic using the conservative variance
estimator from \citet{fogarty2018}, and then compare the resulting
standardized test statistic to the standard Normal distribution to
obtain \(p\)-values.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# For each Gamma, compute one{-}sided weak{-}null p{-}value}
\NormalTok{weak\_results\_list }\OtherTok{\textless{}{-}} \FunctionTok{lapply}\NormalTok{(}
  \AttributeTok{X =}\NormalTok{ Gamma\_vals,}
  \AttributeTok{FUN =} \ControlFlowTok{function}\NormalTok{(G) \{}

    \CommentTok{\# Compute per{-}set worst{-}case IPW; keep as list to preserve attributes}
\NormalTok{    strat\_stats }\OtherTok{\textless{}{-}}\NormalTok{ data\_matched }\SpecialCharTok{|\textgreater{}}
\NormalTok{      dplyr}\SpecialCharTok{::}\FunctionTok{group\_by}\NormalTok{(fm) }\SpecialCharTok{|\textgreater{}}
\NormalTok{      dplyr}\SpecialCharTok{::}\FunctionTok{summarise}\NormalTok{(}
        \AttributeTok{n      =}\NormalTok{ dplyr}\SpecialCharTok{::}\FunctionTok{n}\NormalTok{(),                      }\CommentTok{\# set size n\_s}
        \AttributeTok{prop\_n =}\NormalTok{ n }\SpecialCharTok{/} \FunctionTok{nrow}\NormalTok{(data\_matched),          }\CommentTok{\# weight n\_s / n}
        \AttributeTok{est    =} \FunctionTok{list}\NormalTok{(}\FunctionTok{worst\_case\_IPW}\NormalTok{(             }\CommentTok{\# keep as list()}
                         \AttributeTok{z =}\NormalTok{ UN,}
...
\NormalTok{      dplyr}\SpecialCharTok{::}\FunctionTok{mutate}\NormalTok{(}
        \AttributeTok{alt =} \FunctionTok{attr}\NormalTok{(est[[}\DecValTok{1}\NormalTok{]], }\StringTok{"alternative"}\NormalTok{),      }\CommentTok{\# read attribute once}
        \AttributeTok{est =} \FunctionTok{as.numeric}\NormalTok{(est)                      }\CommentTok{\# unlist to numeric}
\NormalTok{      )}

    \CommentTok{\# Weighted statistic and variance}
...
\NormalTok{    )}
    ...
    \CommentTok{\# z: only if variance finite and positive; else NA}
...
\NormalTok{    z }\OtherTok{\textless{}{-}} \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{is.finite}\NormalTok{(denom) }\SpecialCharTok{\&\&}\NormalTok{ denom }\SpecialCharTok{\textgreater{}} \DecValTok{0}\NormalTok{) num }\SpecialCharTok{/}\NormalTok{ denom }\ControlFlowTok{else} \ConstantTok{NA\_real\_}
    ...
    \CommentTok{\# Tail from preserved attribute (same for all rows)}
\NormalTok{    alt }\OtherTok{\textless{}{-}}\NormalTok{ strat\_stats}\SpecialCharTok{$}\NormalTok{alt[}\DecValTok{1}\NormalTok{]}
\NormalTok{    lower\_tail }\OtherTok{\textless{}{-}} \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{identical}\NormalTok{(alt, }\StringTok{"greater"}\NormalTok{)) }\ConstantTok{FALSE} \ControlFlowTok{else} \ConstantTok{TRUE}

    \CommentTok{\# One{-}sided p{-}value}
\NormalTok{    pval }\OtherTok{\textless{}{-}} \ControlFlowTok{if}\NormalTok{ (}\FunctionTok{is.na}\NormalTok{(z)) }\ConstantTok{NA\_real\_} \ControlFlowTok{else} \FunctionTok{pnorm}\NormalTok{(z, }\AttributeTok{lower.tail =}\NormalTok{ lower\_tail)}

    \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{Gamma =}\NormalTok{ G, }\AttributeTok{p\_value =}\NormalTok{ pval, }\AttributeTok{stringsAsFactors =} \ConstantTok{FALSE}\NormalTok{)}
\NormalTok{  \}}
\NormalTok{)}
...
\begin{center}\includegraphics{Building_Design-Based_Matching_Pipeline_files/figure-latex/unnamed-chunk-52-1} \end{center}
\doublespacing
