...
\textbf{Part 1: Making a Matched Dataset (toward Justifying As-If
Randomization)}
...
\textbf{Part 2: Causal Inference (under As-If Randomization)}
...
\textbf{Part 3: Sensitivity Analysis (Departures from As-If
Randomization)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Define base URL for the Matching Guide GitHub repository}
...
\subsection{Part 1: Making a Matched Dataset (toward Justifying As-If
Randomization)}\label{part-1-making-a-matched-dataset-toward-justifying-as-if-randomization}
...
\NormalTok{data }\OtherTok{\textless{}{-}}\NormalTok{ data }\SpecialCharTok{|\textgreater{}}  \CommentTok{\# Pipe (|\textgreater{}) to pass left{-}hand result into next function call}
  \FunctionTok{mutate}\NormalTok{(}
    ...
    \CommentTok{\# ifelse() returns one value if the condition is TRUE and another if FALSE}
...
\CommentTok{\# New character vector of covariate names with "region" replcaed by dummy indicators }
...
\FunctionTok{fullmatch}\NormalTok{(}
  ...
  \AttributeTok{mean.controls =} \ConstantTok{NULL}\NormalTok{, }\CommentTok{\# Governs average controls per treated unit}
                        \CommentTok{\# Only one of omit.fraction or mean.controls may be non{-}NULL}
  \AttributeTok{data =}\NormalTok{ data}
\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# Distance matrix for propensity score (logit of estimated treatment probability)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Full matching on PS + rank{-}based Mahalanobis + separate Euclidean distances}
\CommentTok{\# (ethfrac, bwgdp) + region exact match}
\NormalTok{fm }\OtherTok{\textless{}{-}} \FunctionTok{fullmatch}\NormalTok{(}
  \CommentTok{\# x specifies the distance structure for matching: it can be}
  \CommentTok{\# (i) a distance{-}specification formula passed to match\_on(),}
  \CommentTok{\# (ii) a precomputed distance matrix, or}
  \CommentTok{\# (iii) as here, a sum of distance specifications constructed via match\_on()}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Extract overall chi{-}square balance test results, stratified by matched set (fm)}
...
\subsection{Part 2: Causal Inference (under As-If
Randomization)}\label{part-2-causal-inference-under-as-if-randomization}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Load the hm\_stat\_rescale() function from the GitHub repo}
\CommentTok{\# Base\_url (defined earlier as}
\CommentTok{\# "https://raw.githubusercontent.com/tl2624/matching{-}guide/main")}
\CommentTok{\# Points to the main GitHub repo URL}
...
\NormalTok{data\_matched }\OtherTok{\textless{}{-}} \FunctionTok{hm\_stat\_rescale}\NormalTok{(}
  ...
  \AttributeTok{outcome =}\NormalTok{ ldur\_tilde, }\CommentTok{\# Set outcome variable to be rescaled within matched sets}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Fixed{-}effects (FE) regression coefficient on UN (matched set indicators as FE)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Install "PSweight" package (only run if you don\textquotesingle{}t already have it installed)}
...
\CommentTok{\# Encode the treatment assignment probabilities implied by as{-}if randomization}
\CommentTok{\# within matched sets: n\_treated / n in each set}
\NormalTok{ps\_fit }\OtherTok{\textless{}{-}} \FunctionTok{PSmethod}\NormalTok{(}
  ...
  \CommentTok{\# Use as.data.frame() to drop tibble class for compatibility with PSmethod()}
...
\NormalTok{assign\_prob }\OtherTok{\textless{}{-}}\NormalTok{ ps\_fit}\SpecialCharTok{$}\NormalTok{e.h[, }\StringTok{"1"}\NormalTok{]  }\CommentTok{\# Column corresponding to treatment level "1"}

\CommentTok{\# Unit{-}level overlap weights implied by these treatment assignment probabilities}
\CommentTok{\# Control units get weight equal to their treatment assignment probability}
\CommentTok{\# Treated units get weight equal to one minus their treatment assignment probability}
...
\CommentTok{\# Apply sum statistic to each assignment column in \textquotesingle{}sim\_assigns\textquotesingle{}}
...
\NormalTok{  ) }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{summarize}\NormalTok{(}
    \AttributeTok{expect   =} \FunctionTok{ev}\NormalTok{(}
      ...
      \AttributeTok{method =} \StringTok{"RK"}              \CommentTok{\# Use formula from Rosenbaum and Krieger (1990)}
\NormalTok{    )}\SpecialCharTok{$}\NormalTok{expect,                    }\CommentTok{\# Null expectation of sum statistic in matched set}
...
\NormalTok{    )}\SpecialCharTok{$}\NormalTok{vari,                      }\CommentTok{\# Null variance of sum statistic in matched set}
    \AttributeTok{.groups  =} \StringTok{"drop"}
\NormalTok{  )}
...
\CommentTok{\# Upper{-}tail confidence set}
\CommentTok{\# (values of tau\_h not rejected by the upper{-}tail test)}
\CommentTok{\# The lower endpoint is the smallest null value tau\_h for which the}
\CommentTok{\# upper{-}tail p{-}value is still \textgreater{}= alpha; any smaller tau\_h would be rejected}
...
\NormalTok{)}
\CommentTok{\# qnorm(p = 1 {-} alpha): standard Normal critical value for upper one{-}sided test}
...
\NormalTok{)}
\CommentTok{\# At the lower endpoint, the upper{-}tail one{-}sided p{-}value equals alpha/2;}
\CommentTok{\# any smaller null value would be rejected by the two{-}sided test}
\CommentTok{\# At the upper endpoint, the lower{-}tail one{-}sided p{-}value equals alpha/2;}
\CommentTok{\# any larger null value would be rejected by the two{-}sided test}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# For each tau\_h value, repeat the same randomization test calculation below;}
\CommentTok{\# sapply() runs this repeatedly and stacks the resulting p{-}values together}
...
\NormalTok{                  dat\_tau\_h }\OtherTok{\textless{}{-}} \FunctionTok{hm\_stat\_rescale}\NormalTok{(}
                    \CommentTok{\# transform(): create copy of data\_matched with}
                    \CommentTok{\# shifted{-}outcome column}
...
\NormalTok{                  )}
                  ...
                  \CommentTok{\# Randomization dis via simulated assignments (defined above)}
                  \CommentTok{\# Compute simulated null dist via matrix multiplication,}
                  \CommentTok{\# which is faster than looping over assignments via apply()}
...
\CommentTok{\# Extract vectors of p{-}values}
...
\CommentTok{\# Upper{-}tail confidence set}
\CommentTok{\# retain all tau\_h with upper{-}tail p \textgreater{}= alpha}
...
\subsection{Part 3: Sensitivity Analysis (Departures from As-If
Randomization)}\label{part-3-sensitivity-analysis-departures-from-as-if-randomization}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Rosenbaum (2018) objective: expectation minus observed plus kappa times SD}
\CommentTok{\# Nonpositive values imply rejection at level alpha}
\NormalTok{sens\_objective }\OtherTok{\textless{}{-}} \ControlFlowTok{function}\NormalTok{(null\_expect, null\_variance, obs\_stat, }\AttributeTok{alpha =} \FloatTok{0.05}\NormalTok{) \{}
    \CommentTok{\# null\_expect : overall null expectation of the test statistic}
    \CommentTok{\# null\_variance : overall null variance of the test statistic obs\_stat :}
    \CommentTok{\# observed value of the test statistic alpha : test size (e.g., 0.05 gives}
    \CommentTok{\# kappa about 1.64)}

    \CommentTok{\# Normal critical value}
\NormalTok{    kappa }\OtherTok{\textless{}{-}} \FunctionTok{qnorm}\NormalTok{(}\AttributeTok{p =} \DecValTok{1} \SpecialCharTok{{-}}\NormalTok{ alpha)}

    \CommentTok{\# Null standard deviation}
\NormalTok{    null\_sd }\OtherTok{\textless{}{-}} \FunctionTok{sqrt}\NormalTok{(null\_variance)}

    \CommentTok{\# Gap between null expectation and observed statistic (null\_expect {-}}
    \CommentTok{\# obs\_stat), plus Normal buffer (kappa * null\_sd)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Evaluate under Gamma = 1 using null\_ev, null\_var, and obs\_stat defined earlier}
...
\CommentTok{\# Iterate over the Gamma\_vals grid using lapply(): perform the sensitivity}
\CommentTok{\# analysis at each Gamma and return a list of per{-}Gamma result objects}
...
\NormalTok{    out }\OtherTok{\textless{}{-}} \FunctionTok{senstrat}\NormalTok{(}
      ...
      \AttributeTok{alternative =} \StringTok{"greater"}\NormalTok{,                         }\CommentTok{\# upper one{-}sided test}
      \AttributeTok{detail      =} \ConstantTok{TRUE} \CommentTok{\# return intermediate quantities used in the computations}
\NormalTok{    )}
...
worst-case procedure replaces \(p(\bm{z}_s)\) with the upper bound from
...
worst-case IPW Difference-in-Means minus the null ATE, divided by the
square root of the estimated variance --- relative to what would be
obtained using the true (but unknown) variance.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Load the worst\_case\_IPW() function from the GitHub repo}
\FunctionTok{source}\NormalTok{(}
  \FunctionTok{paste0}\NormalTok{(}
...
\NormalTok{  \} }\ControlFlowTok{else}\NormalTok{ \{}
    \FunctionTok{stop}\NormalTok{(}\StringTok{"Only one{-}sided alternatives (\textquotesingle{}greater\textquotesingle{} or \textquotesingle{}less\textquotesingle{}) are handled here."}\NormalTok{)}
\NormalTok{  \}}
...
\CommentTok{\# Sensitivity value: smallest Gamma at which the one{-}sided test no longer rejects}
