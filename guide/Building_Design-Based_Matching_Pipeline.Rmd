...
The Mahalanobis distance [@mahalanobis1936] addresses both of these concerns. First, it standardizes covariates so that differences are placed on a comparable scale. This standardization is used solely to compute distances across covariates that typically differ in scale, regardless of the substantively chosen transformations, with the aim of constructing matches that ultimately support apples-to-apples comparisons on these original, substantively chosen scales. Second, the Mahalanobis distance adjusts for correlations among covariates, ensuring that highly related variables are not effectively counted twice. We can therefore construct a distance matrix based on Mahalanobis rather than Euclidean distances as follows.
...
\end{align}
where $\bm{x}_i$ is the covariate vector, $\bm{\hat{\beta}}$ is the vector of estimated coefficients, and the superscript $\top$ denotes the transpose, turning the row vector $\bm{\hat{\beta}}$ into a column vector. The quantity $\bm{\hat{\beta}}^{\top}\bm{x}_i$ is the linear index, and the inverse logistic function, $1 / (1 + \exp(-x))$, maps any real-valued input, $x$, onto the interval $(0, \, 1)$. The linear covariate index for unit $i$, $\bm{\hat{\beta}}^{\top}\bm{x}_i$, is simply the logit, i.e., log odds, transformation of $\hat{\lambda}(\bm{x}_i)$ in \eqref{eq: linear index}.
...
Relaxing calipers allows additional units to be included in the matched design, since units without eligible matches would otherwise be discarded. Including these additional units can increase the effective sample size, but such gains need not be large. In particular, as the formula for a set's harmonic mean in \eqref{eq: harmonic mean} makes clear, the number of treated-versus-control comparisons does not increase because optimally matched sets, as we noted earlier, contain a single treated unit with possibly many controls or a single control unit with possibly many treated units. As a result, the contribution of newly admitted units to the effective sample size is limited. The magnitude of any increase in effective sample size --- given by the sum of harmonic means across sets --- depends on how these newly admitted units are distributed across matched sets: If these units are concentrated within only a few sets, the resulting gain will be modest.
...
Note, however, that the matched design may alter the study population over which these causal targets are defined relative to the original set of units [@rosenbaumrubin1985; @rosenbaum2012]. Units excluded from the matched design --- because they lack suitable matches under the chosen calipers or exact-match constraints --- are no longer part of the study population to which inference applies. Consequently, both $\bm{\tau}$ and $\tau$ refer only to the subpopulation of units retained after matching. This resulting study population is often difficult to interpret. For this reason, @rosenbaum2010 cautions that "it is usually better to go back to the covariates themselves ... perhaps redefining the population under study to be a subpopulation of the original population" (p. 86), a point echoed in @stuart2010. Several approaches explicitly address how to define and interpret these study populations, including methods that trim units with extreme estimated propensity scores --- where overlap is limited [@crumpetal2009; @kingzeng2006] --- as well as approaches developed for matching more generally [@rosenbaum2012; @fogartyetal2016; @traskinsmall2011].

The breadth of an interpretable study population with meaningful treatment–control comparisons depends in part on the relative abundance of treated and control units across covariate space. When either treatment arm is thinly represented in regions of covariate space dominated by the other, overlap may be confined to a restricted region of covariate space and include relatively few units. All else equal, a larger pool of units available for matching supports overlap over a wider region of covariate space and yields a larger, more interpretable study population over which the target of inference is defined.
...
Because the original function is concave, if the maximum value of its tangent-line approximation over $\bm{u} \in \mathcal{U}_{+}$ is at or below zero, then the original function itself must be nonpositive for every $\bm{u} \in \mathcal{U}_{+}$. Therefore, we can safely base the decision to reject the null hypothesis or not on the Taylor approximation. Although valid, this procedure can be conservative: The tangent line may remain above zero even when the original function would be below zero at its true worst case. By contrast, the separable approximation can be liberal, as it may reject even when the worst-case value of the function is positive.
...
sens_results <- lapply(
  ...
  FUN = function(g) {
    ...
    data.frame(
      ...
      p_sep     = p_sep,
      cvA       = cvA,
      agree_tay = (sep_reject == tay_reject)
    )
  }
)
...
For this matched dataset, the conclusions from the separable and Taylor series approximations agree for nearly all values of $\Gamma$, differing only over a narrow interval from `r min(sens_df$Gamma[which(sens_df$agree_tay == 0)])` to `r max(sens_df$Gamma[which(sens_df$agree_tay == 0)])`. We can reject the sharp null of no effect under as-if randomization (i.e., $\Gamma = 1$), but this conclusion is sensitive to departures from that assumption. At a $\Gamma$ of roughly `r round(x = sens_value_tay, digits = 2)`, we are no longer able to reject the sharp null of no effect against the alternative of a larger effect at an $\alpha$-level of $0.05$.

The plot below illustrates this comparison and clarifies how the two approaches relate across values of $\Gamma$

```{r, tidy = FALSE, echo = FALSE, fig.pos = "H", fig.width = 7, fig.height = 4, fig.cap =  "The top panel reports the upper one-sided p-value under the separable approximation, while the bottom panel reports the Taylor-series margin (with zero marking the rejection boundary). The shaded vertical band highlights values of $\\Gamma$ for which the separable approximation and the Taylor-series approximation lead to different rejection decisions."}
...
These sensitivity analyses rely on multiple approximations. For example, the separable approximation is used to approximate the worst-case configuration of hidden bias $\bm{u} \in \mathcal{U}_+$, and inference additionally relies on a Normal approximation to the null randomization distribution of the test statistic. If the exact randomization distribution were used instead, the resulting sensitivity conclusions could differ for certain values of $\Gamma$. In principle, one could assess sensitivity at a given $\Gamma$ using the exact randomization distribution by fixing a configuration of hidden bias $\bm{u}$ --- for example, the separable choice --- and deriving, from \eqref{eq: indiv sens prob}, the implied probability distribution on $\Omega$, which can then be combined with the previously defined exact_sharp_null_dist to compute the corresponding $p$-value.

```{r}

# Load helper that constructs all block-respecting candidate u vectors
source(paste0(base_url, "/R/construct_cand_u.R"))

# U_plus: n x K matrix, each column a candidate u vector in original row order
u_plus <- construct_cand_u(
  set      = data_matched$fm,                  # matched-set IDs
  y_scaled = data_matched$ldur_tilde_hm_scaled # scaled outcomes used for ordering
)$U

```

We now compute the exact worst-case upper-tail $p$-value for the specific value $\Gamma = r Gamma$ by evaluating the upper $p$-value for every $\bm{u} \in \mathcal{U}_+$. For each candidate $\bm{u}$, the probability of each assignment in $\Omega$ is derived from the sensitivity model in \eqref{eq: indiv sens prob}.

```{r}

Gamma <- 1.1161

# Log-odds sensitivity parameter
gamma <- log(Gamma)

# Number of candidate u vectors
K <- ncol(u_plus)

# Initialize worst-case p-value and the column index of the worst u
worst_p     <- -Inf
worst_pos_u <- NA_integer_

# Loop over all block-respecting candidate u's
for (k in seq_len(K)) {
  
  # Candidate u_k (length n, original row order)
  u_vec <- u_plus[, k]
  
  # For each assignment z_j in Omega (columns of exact_assigns),
  # compute u^T z_j; result is a length-B vector
  zu <- as.numeric(crossprod(u_vec, exact_assigns))
  
  # Assignment probabilities under the sensitivity model:
  # P(Z = z_j | u) proportional to exp{ gamma * (u^T z_j) }
  w <- exp(gamma * zu)
  w <- w / sum(w)
  
  # Exact upper-tail p-value for this u_k
  p_val_k <- sum(w[exact_sharp_null_dist >= obs_stat])
  
  # Keep track of the largest (worst-case) p-value and its u_k
  if (p_val_k > worst_p) {
    worst_p     <- p_val_k
    worst_pos_u <- k
  }
...
# Worst-case exact upper-tail p-value at this Gamma
worst_p

# Corresponding worst-case u vector (original row order)
exact_u_worst <- u_plus[, worst_pos_u]

```

At $\Gamma = `r Gamma`$, the worst-case exact upper-tail $p$-value already exceeds $\alpha = 0.05$, so the null hypothesis can no longer be rejected for this degree of departure from as-if randomization. Consequently, the previously reported changepoint --- the smallest value of $\Gamma$ at which the worst-case $p$-value reaches or exceeds $\alpha$ --- of approximately `r round(x = sens_value_tay, digits = 2)` is too large.

To diagnose the source of this discrepancy, we next evaluate the Normal-approximation $p$-value at the true worst-case configuration $\bm{u} \in \mathcal{U}_+$. By holding fixed the reference distribution and varying only the choice of $\bm{u}$, this calculation isolates the role played by the search over hidden-bias configurations from the role played by the Normal approximation itself. Comparing the resulting $p$-value to the separable result therefore clarifies whether the discrepancy arises from approximation error in selecting $\bm{u}$ or from the Normal approximation to the randomization distribution.

```{r}

# Initialize worst-case Normal-approximation p-value over u in U+
worst_p_norm <- -Inf

# Loop over all candidate u's
for (k in seq_len(K)) {
  
  # Candidate u_k (original row order)
  u_vec <- u_plus[, k]
  
  # Initialize total null mean and variance of the test statistic for this u
  null_mean <- 0
  null_var  <- 0
  
  # Loop over matched sets to accumulate block-level moments
  for (s in levels(data_matched$fm)) {
    
    # Indices for units in stratum s
    rows_s <- which(data_matched$fm == s)
    
    # Number of ones in u within this stratum
    m_s <- sum(u_vec[rows_s])
    
    # RK block-level expectation and variance for given m_s and Gamma
    moms <- ev(
      sc     = data_matched$ldur_tilde_hm_scaled[rows_s],
      z      = data_matched$UN[rows_s],
      m      = m_s,
      g      = Gamma,
      method = "RK"
    )
    
    # Add block-level contributions to overall mean and variance
    null_mean <- null_mean + moms$expect
    null_var  <- null_var  + moms$vari
  }
  
  # Standardized test statistic under this u (Normal approximation)
  z_stat <- (obs_stat - null_mean) / sqrt(null_var)
  
  # Upper-tail Normal p-value for this u
  p_val_norm <- as.numeric(pnorm(z_stat, lower.tail = FALSE))
  
  # Track the largest (worst-case) Normal p-value across u
  if (p_val_norm > worst_p_norm) {
    worst_p_norm <- p_val_norm
  }
}

# Match the row in sens_df closest to this Gamma (guards against
# floating-point issues when comparing Gamma values)
gamma_row <- which.min(abs(sens_df$Gamma - Gamma))

# Compare Normal approximation at the true worst-case u to
# the separable and Taylor-series Normal approximations
data.frame(
  Gamma        = Gamma,
  p_worst_norm = worst_p_norm,                # Normal p-value at true worst-case u
  p_sep        = sens_df$p_sep[gamma_row]     # separable approximation p-value
)

```

This comparison shows that, in this example, the Normal-approximation $p$-value evaluated at the true worst-case configuration $\bm{u}$ is essentially identical to the separable-approximation $p$-value. Thus, the difference between the exact and approximate sensitivity analyses is attributable to the Normal approximation to the randomization distribution, rather than to the separable approximation used to select $\bm{u}$.
...
- Sixth, we have not incorporated into the overall pipeline an explicit approach for defining an **interpretable study population** in matched designs. This omission matters because discarding units during matching can alter the population over which the causal estimand is defined [@rosenbaumrubin1985; @rosenbaum2012]. Common practice excludes units based on limited overlap in the estimated propensity score [@crumpetal2009; @kingzeng2006], but study populations defined in this way can be difficult to characterize substantively. This concern has motivated alternative approaches that define the study population directly in terms of a small number of covariates. For example, @traskinsmall2011 use a classification tree to define a study population that approximates one defined by trimming on the estimated propensity score, while @fogartyetal2016 use discrete optimization to define a maximal, rectangular region of covariate space with adequate overlap on key covariates.
