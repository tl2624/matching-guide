...
The basic logic of this approach is as follows. For a fixed value of $\Gamma$, @rosenbaum2018 constructs a function that, for each possible configuration of hidden confounding $\bm{u} \in \mathcal{U}_{+}$, quantifies how far the observed test statistic lies beyond the upper one-sided Normal rejection threshold implied by that value of $\bm{u}$ under the null hypothesis. That is, call the null expectation of the test statistic plus the critical value times the test statistic's null standard deviation the "cutoff," above which our observed test statistic would be significant.

In our example, when we choose the separable approximation $\bm{u} \in \mathcal{U}_+$, the expected value of the harmonic mean weighted Difference-in-Means under the null is approximately `r round(x = null_ev, digits = 4)`, and the corresponding standard deviation is approximately `r round(x = sqrt(null_var), digits = 4)`. At significance level $\alpha = 0.05$, the Normal cutoff is therefore approximately `r round(x = null_ev + qnorm(p = 0.95) * sqrt(null_var), digits = 4)`. When our observed test statistic falls in the rejection region when it exceeds this cutoff. So we can simply take the cutoff of `r round(x = null_ev + qnorm(p = 0.95) * sqrt(null_var), digits = 4)` minus the observed test statistic of `r round(x = obs_stat, digits = 4)` and assess whether it is less than or equal to 0 (implying rejection) or not.

Thinking about hypothesis testing in terms of this function --- cutoff minus observed test statistic --- is valuable because this function is concave, which means that if you draw its tangent line at any point, the line always lies above the function itself over all $\bm{u} \in \mathcal{U}_{+}$. Therefore, if we use a linear approximation to this function, we can evaluate the linear approximation itself as a global upper bound on how large the original function could be. This linear approximation is much easier to work with because linear functions can be optimized within each strata and then stitched back together. Because of concavity of the original function, if even this upper bound is at or below zero, then the original function must be nonpositive for every possible configuration of hidden confounding, implying rejection of the null hypothesis.

Although valid, this approach is conservative because sometimes the linear approximation stays above $0$, even though the original function would be below $0$ at its worst-case $\bm{u} \in \mathcal{U}_{+}$. In those situations, the test will say "do not reject," even though rejection would have been justified. The separable approximation, by contrast, can be liberal: It may lead to rejection even when the function at its worst-case $\bm{u} \in \mathcal{U}_+$ is positive.
