...
The basic logic of this approach is as follows. For a fixed value of $\Gamma$, @rosenbaum2018 constructs a function $\lambda(\bm{u};\Gamma)$ that, for each possible configuration of hidden confounding $\bm{u}$, quantifies how far the observed test statistic lies beyond the upper one-sided Normal rejection threshold implied by $\bm{u}$ under the null hypothesis. Formally,
\begin{equation}
\label{eq: lambda}
\lambda(\bm{u};\Gamma) \coloneqq
\underbrace{\E_{\bm{u},\Gamma}\left[T(\bm{Z}; \tau_h)\right]}_{\text{Null expectation}} + \underbrace{\Phi^{-1}(1 - \alpha)}_{\text{Upper one-sided Normal critical value}} \underbrace{\sqrt{\Var_{\bm{u},\Gamma}\left[T(\bm{Z}; \tau_h)\right]}}_{\text{Null standard deviation}} - \underbrace{T^{\mathrm{obs}}(\tau_h)}_{\text{Observed statistic}} .
\end{equation}
For an upper one-sided test at significance level $\alpha$, the quantity $\E_{\bm{u},\Gamma}[T(\bm{Z};\tau_h)] + \Phi^{-1}(1-\alpha)\sqrt{\Var_{\bm{u},\Gamma}[T(\bm{Z};\tau_h)]}$ is the Normal rejection cutoff under the null. For example, when $\alpha = 0.05$, this cutoff lies approximately $1.64$ standard deviations above the null expectation. Consequently, $\lambda(\bm{u};\Gamma) \leq 0$ if and only if the observed test statistic exceeds this cutoff, in which case the null hypothesis is rejected.

For a given $\Gamma$, we would reject the null, i.e., the function in \eqref{eq: lambda} would be non-positive, over all possible $\bm{u}$ if \eqref{eq: lambda} is non-positive under the $\bm{u}$ that makes the value of the function largest. Thus, to determine rejection of the null or not, we can assess whether \eqref{eq: lambda} is non-positive at the worst-case $\bm{u}$. The $\bm{u}$ returned by the separable approximation may not truly be the worst-case $\bm{u}$, but instead of finding trying to find the worst-case $\bm{u}$, @rosenbaum2018 shows that the function in \eqref{eq: lambda} is concave, which means that if you draw its tangent line at any point, the line always lies above the curve itself.

The separable configuration of hidden confounding serves as the point at which we construct a linear approximation to $\lambda(\bm{u};\Gamma)$. This linear function coincides with $\lambda(\bm{u};\Gamma)$ at the separable configuration and, because $\lambda(\bm{u};\Gamma)$ is concave, lies above it for all other configurations of $\bm{u}$. We therefore evaluate the linear approximation itself as a global upper bound on how large $\lambda(\bm{u};\Gamma)$ could be. If even this upper bound is at or below zero, then $\lambda(\bm{u};\Gamma)$ must be nonpositive for every possible configuration of hidden confounding, implying rejection of the null hypothesis.
