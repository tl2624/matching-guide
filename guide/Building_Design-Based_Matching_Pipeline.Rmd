...
```{r, echo = FALSE, fig.pos = "H", fig.width = 5, fig.height = 3, fig.cap = "The empirical distributions of the linear covariate index for treated and control groups"}
...
Note, however, that the matched design may alter the study population over which these causal targets are defined relative to the original set of units [@rosenbaumrubin1985; @rosenbaum2012]. Because units that fail to satisfy the specified caliper or exact-match constraints are excluded from the matched design, both $\bm{\tau}$ and $\tau$ pertain only to the subpopulation of units retained after matching. This resulting study population is often difficult to interpret. For this reason, @rosenbaum2010 cautions that "it is usually better to go back to the covariates themselves ... perhaps redefining the population under study to be a subpopulation of the original population" (p. 86), a point echoed in @stuart2010. Several approaches explicitly address how to define and interpret these study populations, including methods that trim units with extreme estimated propensity scores --- where overlap is limited [@crumpetal2009; @kingzeng2006] --- as well as approaches developed for matching more generally [@rosenbaum2012; @fogartyetal2016; @traskinsmall2011].
...
```{r, tidy = FALSE, echo = FALSE, fig.pos = "H", fig.width = 6.5, fig.height = 3.5, fig.cap = "One-sided upper $p$-values for the weak null hypothesis of zero average effect across values of the sensitivity parameter $\\Gamma$"}
...
- Seventh, we have not considered how matched designs can leverage an additional control group [@rosenbaum1987b] or outcomes with known effects [@rosenbaum1989], which can be used to strengthen robustness to violations of as-if randomization, to guide plausible sensitivity analyses [@rosenbaum2023a], or to construct evidence factors [@rosenbaum2023b].
