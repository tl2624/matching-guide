...
We can see in \texttt{R} that this observed sum statistic coincides with the harmonic-mean–weighted Difference-in-Means computed using \texttt{optmatch}’s harmonic weights, and also with the coefficient on the treatment indicator from a fixed-effects regression that includes matched-set indicators.
...
dim_hm <- data_matched |>
  ...
  summarize(
    ...
    # Harmonic-mean weight for the set (equals its contribution to effective sample size)
...
# Compare both to the fixed-effects regression coefficient on UN
fe_fit <- lm(formula = ldur_tilde ~ UN + fm,
             data = data_matched)

# Drop the coefficient name ("UN") so values compared numerically, not by names
all.equal(dim_hm, unname(coef(fe_fit)["UN"]), obs_stat)
