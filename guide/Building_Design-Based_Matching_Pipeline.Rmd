...
Although this is rarely feasible in practice, our running example is small enough that we can enumerate the full set of ca configurations, $\mathcal{U}_+$, and then compute the corresponding exact (or Normal-approximation-based) worst-case $p$-value for a fixed sensitivity parameter $\Gamma$.

We begin by constructing the candidate set $\mathcal{U}_+$, consisting of all vectors $\bm{u}$ that respect the matched-set structure and assign higher confounding values to units with larger outcomes within each matched set.

```{r}

u_plus <- construct_cand_u(
  set      = data_matched$fm,
  y_scaled = data_matched$ldur_tilde_hm_scaled
)$U

```

We now compute the exact worst-case upper-tail $p$-value for the specific value $\Gamma = `r Gamma`$ by explicitly evaluating the upper $p$-value for every $\bm{u} \in \mathcal{U}_+$. For each candidate $\bm{u}$, the probability of each assignment in $\Omega$ can be calculated from the distribution implied by \eqref{eq: indiv sens prob} [see @rosenbaum2002].
...
# Initialize worst-case p-value and position
worst_p      <- -Inf
...
for (k in seq_len(K)) {
  ...
  # Compute z_j^T u_k for all assignments z_j
  # exact_assigns is n x B
  zu <- as.numeric(crossprod(u_vec, exact_assigns))  # length B
  
  # Assignment probabilities:
  # Pr(Z = z_j | u_k) proportional to exp(gamma * z_j^T u_k)
  w <- exp(gamma * zu)
  w <- w / sum(w)
  ...
  # Update worst-case p-value and location
  if (p_val_k > worst_p) {
    worst_p     <- p_val_k
    worst_pos_u <- k
  }
...
# Worst-case exact upper-tail p-value
worst_p
...
At this value of $\Gamma = `r Gamma`$, the worst-case upper-tail $p$-value already exceeds $\alpha = 0.05$. This implies that the changepoint values of `r sens_value_tay`, obtained from the Taylor series approximation, and `r sens_value_sep`, obtained from the separable approximation, are too large.

To diagnose the source of this discrepancy, we repeat the same worst-case search over $\mathcal{U}_+$ using the Normal approximation to the null randomization distribution. In contrast to the exact calculation above, this approach replaces the exact randomization distribution with a Normal approximation whose mean and variance depend on $\bm{u}$ through block-level expectations and variances.

```{r}

# Initialize worst-case Normal-approximation p-value
worst_p_norm <- -Inf
...
for (k in seq_len(K)) {
  ...
  # Initialize null mean and variance for this u
  null_mean <- 0
  null_var  <- 0
  ...
  for (s in levels(data_matched$fm)) {
    ...
    # Accumulate block-level moments
    null_mean <- null_mean + moms$expect
    null_var  <- null_var  + moms$vari
  }
  
  # Normal-approximation upper-tail p-value
  z_stat <- (obs_stat - null_mean) / sqrt(null_var)
  p_val_norm <- as.numeric(pnorm(z_stat, lower.tail = FALSE))
  
  # Update worst-case Normal p-value (force scalar)
  if (p_val_norm > worst_p_norm) {
    worst_p_norm <- p_val_norm
  }
}

# Combine all three p-values for comparison
data.frame(
  Gamma       = Gamma,
  p_worst_norm  = worst_p_norm,
  p_sep       = sens_df[sens_df$Gamma == Gamma_target, ]$p_sep,
  p_tay       = sens_df[sens_df$Gamma == Gamma_target, ]$p_tay
)

```

Comparing the calculations, the discrepancy appears to stem from the Normal approximation itself rather than from the separable or Taylor-series approximations for the worst-case $\bm{u}$, since using the true worst-case $\bm{u}$ yields essentially the same Normal-approximation $p$-value.
