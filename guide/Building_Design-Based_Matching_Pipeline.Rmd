...
To load the data, you could first download the replication files from the supplementary information on the article's webpage (DOI: [10.1561/100.00007051](https://doi.org/10.1561/100.00007051)), save the files to your working directory, and then use a package such as \texttt{haven} to load the Stata (.dta) file, \texttt{peace\_pre\_match.dta}, into \texttt{R}. However, for our purposes we recommend loading our pre-created .RData file (\texttt{peace\_pre\_match.RData}), in which Stata’s monthly numeric dates have been converted to R's year-month format, the geographic region indicators recoded into a single factor variable (\texttt{region}), and the ethnic fractionalization index (\texttt{ethfrac}) rescaled from 0-100 to 0-1. The command below loads this pre-created dataset (\texttt{peace\_pre\_match.RData}) into the \texttt{R} environment as an object named \texttt{data}.
...
data <- read_rds(file = "../data/peace_pre_match.rds")
...
We next source a helper function that performs the rescaling described above, ensuring that the sum of the rescaled outcomes across treated units equals the harmonic-mean–weighted Difference-in-Means statistic.

```{r, tidy = FALSE}

# Load the hm_stat_rescale() function
# Check if the function is already loaded; if not, load it below
if (!exists("hm_stat_rescale", mode = "function")) {
  
  # If the function file exists locally, load it from the R/ folder
  if (file.exists("../R/hm_stat_rescale.R")) {
    sys.source(file = "../R/hm_stat_rescale.R",
               envir = environment())  # make it available in the current session
  
  # Otherwise, download and load it directly from the GitHub repository
  } else {
    sys.source(file = paste0(
      "https://raw.githubusercontent.com/tl2624/",
      "matching-guide/main/R/hm_stat_rescale.R"
    ),
    envir = environment()
    )
  }
...
We next apply the rescaling procedure and calculate the observed test statistic.

```{r, tidy = FALSE}

# Apply the rescaling function: adds a new column (.hm_scaled)
# and returns the full matched dataset with this rescaled outcome
data_matched <- hm_stat_rescale(data = data_matched,
                                outcome = ldur_tilde,
                                treat = UN,
                                strata = fm)
...
We can also estimate the variance of the Difference-in-Means in a finely stratified design using the approach of @fogarty2018. Rather than defining the function within the script, we load a custom \texttt{R} function (\texttt{fine\_strat\_var\_est()}) from the accompanying replication files. If the local file is available, the function is sourced directly from the \texttt{R/} folder; otherwise, it is automatically retrieved from the online repository.

```{r, tidy = FALSE}

# Load the fine_strat_var_est() function
# Check if the function is already loaded; if not, load it below
if (!exists("fine_strat_var_est", mode = "function")) {
  
  # If the function file exists locally, load it from the R/ folder
  if (file.exists("../R/fine_strat_var_est.R")) {
    sys.source(file = "../R/fine_strat_var_est.R",
               envir = environment())  # make it available in the current session
  ...
  # Otherwise, download and load it directly from the GitHub repository
    ...
    sys.source(file = paste0(
      "https://raw.githubusercontent.com/tl2624/",
      "matching-guide/main/R/fine_strat_var_est.R"
    ),
    envir = environment()
    )
  }
...
We can also form 95% confidence sets by inverting these hypothesis tests. In this approach, we retain all null values that are not rejected at the $\alpha = 0.05$ level. However, for a two-sided test, we instead use $\alpha/2 = 0.025$ for each tail to account for testing every null value twice (once against each alternative).
...
sens_results <- lapply(
  X = Gamma_vals,
  FUN = function(g){
    out <- senstrat(sc = data_matched$ldur_tilde_hm_scaled, # outcome
...
# Load the worst_case_IPW() function
# Check if the function is already loaded; if not, load it below
if (!exists("worst_case_IPW", mode = "function")) {
  
  # If the function file exists locally, load it from the R/ folder
  if (file.exists("../R/worst_case_IPW.R")) {
    sys.source(file = "../R/worst_case_IPW.R",
               envir = environment())  # make it available in the current session
  ...
  # Otherwise, download and load it directly from the GitHub repository
    ...
    sys.source(file = paste0(
      "https://raw.githubusercontent.com/tl2624/",
      "matching-guide/main/R/worst_case_IPW.R"
    ),
    envir = environment()
    )
  }
  ...
  FUN = function(G) {
    ...
    strat_stats <- data_matched |>
      ...
      dplyr::mutate(
        ...
        est = as.numeric(est)                      # unlist to numeric
      )
