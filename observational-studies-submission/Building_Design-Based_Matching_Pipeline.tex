...
% Define colors for R syntax highlighting
...
\heading{00}{2026}{57}{}{}{Thomas Leavitt and Luke W. Miratrix}

% Short headings should be running head and authors last names

\ShortHeadings{Building a Design-Based Matching Pipeline}{Leavitt and Miratrix}
\firstpageno{1}
...
Practical Implementation in \texttt{R}}
...
\author{\name Thomas Leavitt \email thomas.leavitt@baruch.cuny.edu \\
       ...
       \AND
       \name Luke W. Miratrix \email luke\_miratrix@gse.harvard.edu \\
       \addr Harvard Graduate School Of Education \\
       Harvard University \\
...
\href{https://github.com/tl2624/matching-guide}{GitHub repository (repo)}.

\begin{figure}[h!]

{\centering \includegraphics[width=1\linewidth]{fig/version_2/matching_pipeline_flowchart_V2}
...
Before turning to the first stage, we introduce a running example taken from
...
\subsection{First Stage: Making a Matched Dataset (toward Justifying As-If
...
{\centering \includegraphics[width=\linewidth]{Building_Design-Based_Matching_Pipeline_files/figure-latex/unnamed-chunk-11-1}
...
Finally, we combine the two constraints into distance matrices defined within each region by ``adding'' the objects \texttt{em\_region} and \texttt{euc\_dist\_polity\_cal\_2}, as shown below.
...
\bottomrule
\end{tabular}}
\caption{Adjusted covariate means for treated and control groups and standardized treated--control differences, before and after matching. Asterisks denote two-tailed $p$-values less than or equal to 0.05.}
\label{tab: balance}
\end{table}
...
\item
  ...
  coefficient on the treatment indicator from a fixed-effects (FE) regression
...
We now source a helper function from the companion GitHub repo;
...
the treatment indicator from a FE regression that includes
...
from the companion GitHub repo.
...
from the GitHub repo an \texttt{R} function that, for a fixed
